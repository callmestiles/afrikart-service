...
export async function processWebhookEvent(
  ...
  // All-or-nothing: if the status update or event write fails, the
  // processed_webhooks insert rolls back too — so redelivery is handled correctly
  const processEvent = db.transaction(() => {
    ...
    if (!isNew) {
      console.log(
        `[webhook] Duplicate delivery detected — skipping: ${eventId} (${event})`,
      );
      return { skipped: true };
    }
...
function handleCollectionSuccessful(
  ...
  if (!order) {
    console.warn(
      `[webhook] collection.successful — order not found: ${orderReference}`,
    );
    return;
  }
  ...
  if (order.status !== "pending" && order.status !== "draft") {
    console.log(
      `[webhook] collection.successful — order already in status: ${order.status}, skipping`,
    );
    return;
  }
  ...
  console.log(
    `[webhook] Order ${order.orderId} collected — reference: ${orderReference}`,
  );
}
...
function handlePayoutSuccessful(data: Record<string, unknown>): void {
  ...
  const db = getDb();
  const row = db
    .prepare("SELECT * FROM orders WHERE payout_reference = ?")
    .get(fincraReference) as { id: string; order_id: string } | undefined;

  if (!row) {
    console.warn(
      `[webhook] payout.successful — no order found for payout ref: ${fincraReference}`,
    );
    return;
  }
  ...
  const recipientName = (data.recipient as Record<string, unknown>)
    ?.name as string;
...
function handlePayoutFailed(data: Record<string, unknown>): void {
  ...
  if (!row) {
    console.warn(
      `[webhook] payout.failed — no order found for payout ref: ${fincraReference}`,
    );
    return;
  }

  const reason =
    (data.reason as string) ?? "Destination bank rejected the payout";
  ...
  console.log(
    `[webhook] Payout failed for order ${row.order_id} — reason: ${reason}`,
  );
}
...
function handleChargebackCreated(data: Record<string, unknown>): void {
  ...
  if (!order) {
    console.warn(
      `[webhook] chargeback.created — no order found for ref: ${paymentReference}`,
    );
    return;
  }
  ...
  console.log(
    `[webhook] Chargeback on order ${order.orderId} — amount: ${currency} ${amount}`,
  );
}
